...
//         grpc.WithUnaryInterceptor(opentracingrpc.OpenTracingClientInterceptor(tracer)))
//
...
//         address,
//         ...,  // (existing DialOptions)
//         grpc.WithStreamInterceptor(opentracingrpc.OpenTracingStreamClientInterceptor(tracer)))
//
// All gRPC client spans will inject the OpenTracing SpanContext into the gRPC
// metadata; they will also look in the context.Context for an active
// in-process parent Span and establish a ChildOf reference if such a parent
